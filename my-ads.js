...
function sortAds() {
    ...
    // TOP inzeráty vždy první; uvnitř TOP řadit chronologicky dle createdAt (starší dříve)
    const toDate = (d) => new Date(d?.toDate?.() || d);
    const top = sortedAds.filter(a => !!a.isTop).sort((a, b) => toDate(a.createdAt) - toDate(b.createdAt));
    const rest = sortedAds.filter(a => !a.isTop);
    sortedAds = [...top, ...rest];
...
async function updateAd() {
    try {
		...
		// Nastavit/plánovat smazání pro neaktivní inzeráty (90 dní), jinak zrušit
		if ((updateData.status || '').toString() === 'inactive') {
			updateData.deleteAt = new Date(Date.now() + 90 * 24 * 60 * 60 * 1000);
		} else {
...
async function toggleAdStatus(adId, targetStatus) {
    try {
		...
		// Plánované smazání za 90 dní u neaktivních
		if (newStatus === 'inactive') {
			updatePayload.deleteAt = new Date(Date.now() + 90 * 24 * 60 * 60 * 1000);
		} else {
			updatePayload.deleteAt = null;
		}
		
		await updateDoc(doc(window.firebaseDb, 'users', window.firebaseAuth.currentUser.uid, 'inzeraty', adId), updatePayload);

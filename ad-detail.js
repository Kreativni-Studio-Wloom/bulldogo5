...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // Napojení report modalu (fallback k inline onclick)
    try {
        ...
        console.warn('Report button wiring warning:', e);
    }
});
...
window.openReportModal = function() {
    ...
    if (!viewer) {
        if (typeof window.showAuthRequiredModal === 'function') {
            ...
            alert('Pro nahlášení se prosím přihlaste nebo registrujte.');
        }
        return;
    }
...
window.submitAdReport = function() {
    try {
        const reason = (document.getElementById('reportReason')?.value || '').trim();
        if (!reason) {
            (typeof showMessage === 'function') ? showMessage('Uveďte prosím důvod nahlášení.', 'error') : alert('Uveďte prosím důvod nahlášení.');
            return;
        }
        // Vyžaduje přihlášení (bez toho neodesílat)
        const viewer = window.firebaseAuth?.currentUser;
        if (!viewer) {
            if (typeof window.showAuthRequiredModal === 'function') {
                window.showAuthRequiredModal();
            } else {
                (typeof showMessage === 'function') ? showMessage('Pro nahlášení se prosím přihlaste.', 'error') : alert('Pro nahlášení se prosím přihlaste.');
            }
            return;
        }
        const adTitle = currentAd?.title || 'Bez názvu';
        const adId = currentAd?.id || new URLSearchParams(window.location.search).get('id') || '';
        const ownerId = currentAd?.userId || new URLSearchParams(window.location.search).get('userId') || '';
        const pageUrl = window.location.href;
        const reporter = viewer.email || viewer.uid || 'neznámý';
        
        const subject = `Nahlášení inzerátu: ${adTitle} (${adId})`;
        const bodyLines = [
            `Někdo nahlásil inzerát na Bulldogo.cz`,
            ``,
            `Důvod:`,
            `${reason}`,
            ``,
            `Inzerát: ${pageUrl}`,
            `Ad ID: ${adId}`,
            `Vlastník (userId): ${ownerId}`,
            `Odesílatel: ${reporter}`,
            `Prohlížeč: ${navigator.userAgent}`
        ];
        const body = bodyLines.join('\n');
        
        // Odeslat přes e-mail klienta
        const mailto = `mailto:info@wloom.eu?subject=${encodeURIComponent(subject)}&body=${encodeURIComponent(body)}`;
        window.location.href = mailto;
        
        closeReportModal();
        (typeof showMessage === 'function') ? showMessage('Děkujeme za nahlášení. Podpora byla upozorněna.', 'success') : alert('Děkujeme za nahlášení. Podpora byla upozorněna.');
    } catch (e) {
        console.error('submitAdReport error', e);
        (typeof showMessage === 'function') ? showMessage('Nahlášení se nepodařilo odeslat.', 'error') : alert('Nahlášení se nepodařilo odeslat.');
    }
};

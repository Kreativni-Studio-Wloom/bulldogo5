    ...
    try {
        ...
        if (isLocal || siteKey) {
            if (isLocal) {
                ...
                console.log('✅ App Check: debug token aktivován (localhost)');
            }
            if (siteKey || isLocal) {
                ...
                console.warn('⚠️ App Check přeskočen: chybí reCAPTCHA v3 site key.');
            }
        } else {
            console.warn('⚠️ App Check vypnut (není site key, nejde o localhost). Pro produkci nastavte window.FIREBASE_RECAPTCHA_V3_SITE_KEY.');
        }
    } catch (err) {

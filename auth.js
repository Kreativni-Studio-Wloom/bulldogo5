...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Nouzová inline inicializace Firebase (fallback, když firebase-init.js selže/je blokován)
    async function tryInlineFirebaseInit() {
        if (triedInlineFallback) return false;
        triedInlineFallback = true;
        try {
            console.warn('⚠️ Spouštím nouzovou inline inicializaci Firebase (fallback)');
            ...
            // Stejná konfigurace jako ve firebase-init.js
            ...
            console.log('✅ Nouzová inline inicializace Firebase proběhla úspěšně');
            return true;
        } catch (e) {
            console.error('❌ Inline inicializace Firebase selhala:', e);
            return false;
        }
    }
    ...
    setTimeout(() => {
        if (!firebaseAuth || !firebaseDb) {
            ...
            // Pokusit se o nouzovou inline inicializaci, než zobrazíme chybu
            (async () => {
                const ok = await tryInlineFirebaseInit();
                if (ok) {
                    // Po úspěchu spusť znovu init a nepokračuj chybou
                    if (initializeAuthWithFirebase()) {
                        cleanup();
                        return;
                    }
                ...
                cleanup();
                showMessage('Chyba: Firebase se nenačetl. Zkuste obnovit stránku nebo vypněte blokátory.', 'error');
            })();
        }
    }, 15000);
});
...
function initAuth() {
    ...
    import('https://www.gstatic.com/firebasejs/10.7.1/firebase-auth.js').then(({ onAuthStateChanged }) => {
        ...
        onAuthStateChanged(firebaseAuth, (user) => {
            ...
            updateUI(user);
            // Spustit úklid starých neaktivních inzerátů pro přihlášeného uživatele
            try { if (user) cleanupOldInactiveAdsForCurrentUser(); } catch (_) {}
...
function showAuthModal(type = 'login') {
    ...
    // Validace struktury modalu: pokud je starý/nekonzistentní, přestavět
    try {
        const hasRegistrationType = !!modal.querySelector('.registration-type');
        const hasPersonForm = !!modal.querySelector('.person-form');
        const hasCompanyForm = !!modal.querySelector('.company-form');
        const hasAuthForm = !!modal.querySelector('#authForm');
        if (!hasRegistrationType || !hasPersonForm || !hasCompanyForm || !hasAuthForm) {
            console.warn('⚠️ Zjištěn starý/neúplný auth modal – přestavuji na jednotnou verzi');
            modal.remove();
            modal = createAuthModal();
        }
    } catch (_) {}
...
function showAddServiceModal() {
    // Nové chování: přesměrovat na stránku pro vytvoření inzerátu
...
async function addService(serviceData) {
    try {
        ...
        const newRef = await addDoc(collection(firebaseDb, 'users', authCurrentUser.uid, 'inzeraty'), serviceToSave);

        showMessage('Služba byla úspěšně přidána!', 'success');
        try {
            const modal = document.getElementById('addServiceModal');
            if (modal) closeAddServiceModal();
        } catch (_) {}
...
// Úklid: smazat neaktivní inzeráty starší než 90 dní (pouze pro aktuálně přihlášeného uživatele)
async function cleanupOldInactiveAdsForCurrentUser() {
    try {
        ...
        const q = query(adsRef, where('status', '==', 'inactive'));
        const snap = await getDocs(q);
        ...
        let deleted = 0;
        for (const adDoc of snap.docs) {
            const data = adDoc.data() || {};
            const deleteAt = data.deleteAt?.toDate ? data.deleteAt.toDate() : (data.deleteAt ? new Date(data.deleteAt) : null);
            ...
            const referenceDate = updatedAt || createdAt || null;
            const shouldDelete =
                (deleteAt && deleteAt <= now) ||
                (referenceDate && referenceDate <= threshold);
        ...
        if (deleted > 0) {
            console.log(`🧹 Smazáno ${deleted} neaktivních inzerátů starších než 90 dní (uživatel ${currentUser.uid})`);
        }
    } catch (error) {
        console.error('cleanupOldInactiveAdsForCurrentUser: chyba při úklidu', error);
    }
}
    ...
    if (addServiceForm) {
        addServiceForm.addEventListener('submit', async (e) => {
            ...
            const serviceData = {
                ...
                location: formData.get('region') || formData.get('location')
            };
				
				// Validace délky popisu (max 600 znaků)
				const desc = (serviceData.description || '').toString();
				if (desc.length > 600) {
					showMessage(`Popis může mít maximálně 600 znaků (aktuálně ${desc.length}).`, 'error');
					return;
				}
            ...
            if (newId) {
                try {
                    const uid = authCurrentUser?.uid || (window.firebaseAuth?.currentUser?.uid);
                    if (uid) {
                        window.location.href = `ad-detail.html?id=${encodeURIComponent(newId)}&userId=${encodeURIComponent(uid)}`;
                        return;
                    }
                } catch (_) {}
            }
...
// Vytvořit ukázkový TOP inzerát pro aktuálně přihlášeného uživatele
async function createSampleTopAd(days = 7) {
    try {
        const currentUser = window.firebaseAuth?.currentUser;
        if (!currentUser) { showMessage('Přihlaste se pro vytvoření inzerátu.', 'error'); return; }
        if (!window.firebaseDb) { showMessage('Databáze není dostupná.', 'error'); return; }
        ...
        const sample = {
            title: 'Ukázkový TOP inzerát',
            category: 'it_technology',
            description: 'Toto je ukázkový inzerát vytvořený pro test topování.',
            price: '500 Kč/hod',
            location: 'Praha',
            ...
            isTop: true,
            topActivatedAt: now,
            topExpiresAt: expires,
            images: [] // volitelné
        };
        
        const ref = await addDoc(collection(window.firebaseDb, 'users', currentUser.uid, 'inzeraty'), sample);
        console.log('✅ Vytvořen TOP inzerát:', ref.id, sample);
        showMessage('Ukázkový TOP inzerát byl vytvořen.', 'success');
        return ref.id;
    } catch (e) {
        console.error('createSampleTopAd error', e);
        showMessage('Nepodařilo se vytvořit TOP inzerát.', 'error');
    }
}

// Označit existující inzerát jako TOP na X dní
async function markAdAsTop(adId, days = 7) {
    try {
        const currentUser = window.firebaseAuth?.currentUser;
        if (!currentUser) { showMessage('Přihlaste se pro topování inzerátu.', 'error'); return; }
        if (!window.firebaseDb) { showMessage('Databáze není dostupná.', 'error'); return; }
        if (!adId) { showMessage('Chybí ID inzerátu.', 'error'); return; }
        ...
        console.log('✅ Inzerát označen jako TOP:', adId, { topActivatedAt: now, topExpiresAt: expires });
        showMessage('Inzerát byl označen jako TOP.', 'success');
    } catch (e) {
        console.error('markAdAsTop error', e);
        showMessage('Nepodařilo se označit inzerát jako TOP.', 'error');
    }
}
...
// Export helperů pro TOPování
window.createSampleTopAd = createSampleTopAd;
window.markAdAsTop = markAdAsTop;
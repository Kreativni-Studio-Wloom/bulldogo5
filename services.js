...
function initLocalFallback() {
    ...
    try {
        ...
        // TOP služby vždy první; uvnitř TOP chronologicky dle createdAt (starší dříve)
        const top = filteredServices.filter(s => !!s.isTop).sort((a, b) => new Date(a.createdAt) - new Date(b.createdAt));
        const rest = filteredServices.filter(s => !s.isTop);
        ...
        // respektovat stejné řazení jako v online režimu
        sortServices();
        updateStats();
...
function sortServices() {
    ...
    // TOP vždy nahoře; Uvnitř TOP řadit chronologicky dle createdAt (starší dříve)
    const top = base.filter(s => !!s.isTop).sort((a, b) => toDate(a.createdAt) - toDate(b.createdAt));
